...
# =====================
# CSV setup (only used if enabled)
# =====================
def init_csv():
    ...
    if not exists:
        writer.writerow([
            "timestamp", "target_flow", "up", "down", "q",
            "flow_ls", "velocity_ms", "fvel_ms"
        ])
    return f, writer
...
def parse_block(lines, target_flow):
    ...
    return (
        ...
        [
            ...
            target_flow, up, down, q, flow, vel, fvel
        ],
        flow
    )
...
# =====================
def log_data(target_flow, flow_values, time_values, write_csv):
...
def live_plot(target_flow, flow_values, time_values):
    ...
    ax.axhline(target_flow, linestyle="--", linewidth=2,
    ...
    def update(frame):
        ...
        measured_line.set_data(time_values, flow_values)
        ax.relim()
        ax.autoscale_view()
        fig.autofmt_xdate()
        return measured_line,
...
# =====================
def main():
    print("\nFlow Meter Viewer with Optional Logging\n")
    ...
    while True:
        ...
        log_choice = input("Write data to CSV? (y/n): ").strip().lower()
        write_csv = log_choice == "y"
        ...
        reader = threading.Thread(
            target=log_data,
            args=(target, flow_values, time_values, write_csv),
            daemon=True
        )
        reader.start()

        print("\nLive plot running. Type 'stop' and press Enter to end.\n")
